...
// {   ["draw" + (str _x),"onEachFrame"] call BIS_fnc_removeStackedEventHandler;    } foreach unitarray;


/*	[("draw" + (str(o2))),"onEachFrame",{

			if (((floor(player distance o2)) < 1001)) then {
				drawIcon3d["", [0,1,0,(-2*log(floor(player distance o2)/1000))], o2 modelToWorldVisual(o2 selectionPosition "Spine3"), .3, .3, 0, (str(floor(player distance o2))+"M"), 0, .03, "TahomaB"];
			};
